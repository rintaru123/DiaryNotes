...
2. **Установщик (Оптимизированный NSIS)**: Установочный `.exe` файл с максимальным сжатием. Весит значительно меньше и устанавливается на компьютер.

```bash
npm install
npm run electron:build:optimized
```
