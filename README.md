...
Проект настроен на сборку портативного (Portable) десктопного приложения. Программа запускается прямо из папки (идеально для флешки). Все настройки и папки с фото/аудио создаются рядом с файлом приложения.

```bash
npm install
npm run electron:build
```
*После сборки в папке `dist-electron` появится портативный исполняемый `.exe` файл.*
